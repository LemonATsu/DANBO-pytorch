...
def batchify_rays(rays_flat, chunk=1024*64, ray_caster=None, **kwargs):
...
class Trainer:
    ...
    def train_batch(self, batch, i=0, global_step=0):
        ...
        preds = render(H, W, focal, chunk=args.chunk, verbose=i < 10,
                       retraw=False, N_uniques=args.N_sample_images,
    ...
    def compute_loss(self, batch, preds,
                     kp_opts=None, popt_detach=False, global_step=0):
        '''
    ...
    def _compute_nerf_loss(self, batch, rgb_pred, acc_pred, base_bg=1.0,
                           coarse=False, loss_weight=1.0, tag_prefix=''):
        '''compute loss
    ...
    def _compute_lpips_loss(self, batch, rgb_pred, acc_pred, base_bg=1.0,
        ...
        #rgb_pred = F.interpolate(rgb_pred, size=(64,64), mode='bilinear', align_corners=False)
        #rgb_gt = F.interpolate(rgb_gt, size=(64,64), mode='bilinear', align_corners=False)
        ...
        #.mean() * loss_weight * args.lpips_coef

        loss_tag = set_tag_name('lpips_loss', coarse)
        return {loss_tag: lpips_loss}, {}
    ...
    def _compute_soft_softmax_loss(self, preds, global_step):
        args = self.args
    ...
    def save_nerf(self, path, global_step):
        ...
        global_step: global training stepx()
        '''
        args = self.args
        ray_caster = self.render_kwargs_train['ray_caster']
